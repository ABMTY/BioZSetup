    ...
    internal class DeviceManipulator
    {
            ...
            while (objZkeeper.SSR_GetAllUserInfo(machineNumber, out sdwEnrollNumber, out sName, out sPassword, out iPrivilege, out bEnabled))
            {
                for (idwFingerIndex = 0; idwFingerIndex < 10; idwFingerIndex++)
                {
                    if (objZkeeper.GetUserTmpExStr(machineNumber, sdwEnrollNumber, idwFingerIndex, out iFlag, out sTmpData, out iTmpLength))
                    {
                        ...
                        fpInfo.NumeroEquipo = machineNumber;
                        fpInfo.NumeroEnrolamiento = sdwEnrollNumber;
                        fpInfo.Nombre = sName;
                        fpInfo.NumeroDedo = idwFingerIndex;
                        fpInfo.Huella = sTmpData;
                        fpInfo.Privilegio = iPrivilege;
                        fpInfo.IdEmpleado = sPassword;
                        fpInfo.Habilitado = bEnabled;
                        fpInfo.Bandera = iFlag.ToString();
        ...
        public bool UploadFTPTemplate(ZkemClient objZkeeper, int machineNumber, List<UserInfo> lstUserInfo)
        {
            ...
            if (objZkeeper.BeginBatchUpdate(machineNumber, iUpdateFlag))
            {
                ...
                {
                    sdwEnrollNumber = lstUserInfo[i].NumeroEnrolamiento;
                    sName = lstUserInfo[i].Nombre;
                    idwFingerIndex = lstUserInfo[i].NumeroDedo;
                    sTmpData = lstUserInfo[i].Huella;
                    iPrivilege = lstUserInfo[i].Privilegio;
                    sPassword = lstUserInfo[i].IdEmpleado;
                    sEnabled = lstUserInfo[i].Habilitado.ToString();
                    iFlag = Convert.ToInt32(lstUserInfo[i].Bandera);
                    bEnabled = true;
